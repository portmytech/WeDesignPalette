...
fontName=parts[0]||'none';fontWeights=parts[1]||'400,700';fontWeights=fontWeights.split(',');if(fontName!=this.curFont){$('.usof_font_'+this.id).remove();if(fontName=='none'){this.$preview.css('font-family','')}else if($.inArray(fontName,this.notLoadedFonts||[])!==-1){var prepareFontName=fontName;if(this.fontHasGroup(prepareFontName,'uploaded')){prepareFontName='"'+prepareFontName+'"'}
this.$preview.css('font-family',prepareFontName)}else{if(this.curFont!=='get_h1'){$('head').append('<link href="//fonts.googleapis.com/css?family='+fontName.replace(/\s+/g,'+')+'" rel="stylesheet" class="usof_font_'+this.id+'" />')}
