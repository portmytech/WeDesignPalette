		...
		foreach ( $demos as $name => $import ) { ?>
			<div class="w-importer-item" data-demo-id="<?php echo $name; ?>" data-with-force-options="<?php echo (int) isset( $import[ 'force_theme_options' ] ); ?>">
				...
				<div class="w-importer-message done">
					...
					<p><?php echo sprintf( __( 'Just check the result on %syour site%s or start customize via %sTheme Options%s.', 'us' ), '<a href="' . home_url() . '" target="_blank" rel="noopener">', '</a>', '<a href="' . admin_url( 'admin.php?page=us-theme-options' ) . '">', '</a>' ) ?></p>
				</div>
			</div>
		<?php }
		return ob_get_clean();
